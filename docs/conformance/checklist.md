...
| Q50 | A settings reset lands `loadLimit` at the unit's own `maxLoad` on a **second model** — any whose `maxLoad` is not 6; a unit above 1500 W would further separate `maxLoad` from `min(maxLoad, 15)`, which the 600 W observation cannot | disagrees | manual | open | [P-8](#p-8) | [#11](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/11) load limit bounds; Q53 is demonstrated on the 600 W unit, and [#74](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/74) says why the 1000 W run did not demonstrate it |
