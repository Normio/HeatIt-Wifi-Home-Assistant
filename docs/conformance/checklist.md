...
| Q2 | Integer parameters given a decimal (`panelMode=1.0`) are rejected rather than truncated | silent | write | verified fw 1.21 | [#54](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/54) | [#7](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/7) registry serialisation |
...
| Q4 | A multi-parameter write is atomic — one valid plus one out-of-range applies neither | silent | write | verified fw 1.21 | [#54](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/54) | [#7](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/7) one parameter per request |
...
| Q17 | `loadLimit` above the reported `maxLoad` is rejected, not accepted-and-misbehaving | silent | write | verified fw 1.21 | [#54](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/54) | [#11](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/11) load limit bounds |
...
| Q34 | A settings reset leaves `id`, `name`, `room` and the network block untouched, does not reboot, and settles within ~5 s | silent | destructive | verified fw 1.21 | [#74](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/74), [#11](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/11) | [#11](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/11) settings-reset button |
...
| Q50 | A settings reset lands `loadLimit` at the unit's own `maxLoad` on a **second model** — any whose `maxLoad` is not 6; a unit above 1500 W would further separate `maxLoad` from `min(maxLoad, 15)`, which neither probed unit can | disagrees | manual | verified fw 1.21 | [#74](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/74), [P-8](#p-8) | [#11](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/11) load limit bounds; Q53 is demonstrated on the 600 W unit, and [#74](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/74) says why the 1000 W run did not demonstrate it |
...
| Q53 | A settings reset lands every parameter on the vendor document's stated default **except `loadLimit`, which lands on the unit's own `maxLoad`** | disagrees | destructive | verified fw 1.21 | [#74](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/74), [#73](https://github.com/Normio/HeatIt-Wifi-Home-Assistant/issues/73) | `docs/api/heatit-wifi-panel-openapi.yaml` — the only claim we have about post-reset state, and wrong in this one place; Q50 is the same fact from the other side, on a unit whose `maxLoad` is not 6 |
...
**45 verified at firmware 1.21, 13 open.** Nothing is `contradicted` — the ten places the vendor
