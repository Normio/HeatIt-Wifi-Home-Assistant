...
- `diagnostics.py`, the config-entry diagnostics download of spec §7.3 — there
  ...
  data with the host redacted and its options, the parsed status, the firmware
  and whether it is verified, the *observed parameters* and those that have
  vanished, what the last poll did — outcome, duration, whether the status
  read's retry fired — and `raw`: the last status body and headers exactly as
  the panel sent them, through the wire-level scrub `scripts/capture_fixtures.py`
  writes fixtures with. A download from an unverified panel is therefore a
  fixture candidate byte for byte. The README's new `## Diagnostics` section
  says so to the user.
