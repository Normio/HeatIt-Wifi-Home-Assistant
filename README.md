...
the panel's status and writes its settings directly. There is no cloud
...
The integration has been tested on the firmware and panels below. A panel on
another firmware works the same way, it just has not been tested yet. The
details of what was tested are in the
[conformance register](docs/conformance/checklist.md).
