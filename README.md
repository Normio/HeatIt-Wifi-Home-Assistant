...
| Reset energy counter (`reset_energy`) | button | Zeroes the panel's kWh counter, the same counter the MyHeatit app and the panel's display show. **Turned off until you turn it on** from the device page. |
...
The two buttons throw something away — the kWh counter, or every setting — and
Home Assistant has no "are you sure?" for a button, so both arrive turned off
and stay that way until you enable them on the device page. Restoring the
defaults keeps your network and the panel's pairing, and the panel applies it
over about five seconds, so the settings on this page finish catching up a poll
later.
