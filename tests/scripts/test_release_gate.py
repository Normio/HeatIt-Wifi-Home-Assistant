...
chose, deliberately, that both pytest rows block a release.

A called workflow may still carry ``continue-on-error`` — §8.7's ``latest`` row
is a signal on pull requests and the monthly cron — but only as an expression
that is false on a tag. That means the tag guard must appear as one of the
expression's top-level ``&&`` conjuncts. Anything else is rejected, including a
literal and any expression containing ``||``: an ``or`` can be true on a tag
...
#: The conjunct that turns a signal into a blocker on a tag. ``github.ref`` is
#: the caller's ref inside a reusable workflow, so on a release run it is the
#: pushed tag.
    ...
    for name in CALLED_WORKFLOWS:
        ...
        for line in text.splitlines():
            ...
            assert _is_guarded(match.group("value")), (
                ...
                f"top-level && conjunct"
            )


def test_the_guard_recogniser_is_fail_closed() -> None:
    """The recogniser accepts only expressions that a tag actually turns off."""
    ...
    # A negated group is not a top-level conjunct, so it is not recognised.
